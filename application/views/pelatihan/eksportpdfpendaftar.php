...
$today = date("Y-m-d");
$sqldetil = $this->db->query("select daftar_pelatihan.*,user.nama_user, user.email_user, user.alamat_user,pencari_kerja.*,pendidikan.nama_pendidikan 
...
$row=$sqldetil->row();
$configx = $this->db->query("select daftar_pelatihan.*,pelatihan.*,user.nama_user from daftar_pelatihan
...
foreach($sqldetil->result() as $d){
        ...
        $pdf->Cell(35,6,$d->nik_pencaker,1,0);
        $pdf->Cell(30,6,$d->nama_pendidikan,1,0);
        $pdf->Cell(110,6,$d->alamat_user,1,0);
        $pdf->Cell(50,6,$d->email_user,1,1);
}

$pdf->SetFont('Times','',10);

$pdf->Output('Data Pendaftar Pelatihan.pdf', 'I');