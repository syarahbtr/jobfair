    ...
    <div class="section-body">
      ...
      <div class="card">
        ...
        <div class="card-body table-responsive">
          <a href="perusahaan/eksportpdfriwayatlokerln" class="btn btn-primary mb-3"><i class="fa fa-download"></i> Eksport PDF</a>
          <Table class="table table-hover dttable">
            ...
            <tbody>
              ...
              where lowongan.jenis_lowongan='Luar Negeri' and lowongan.tgl_seleksi_wawancara<='$today' and lowongan.jenis_lowongan='Luar Negeri' ");
              foreach ($q->result() as $row) {
                $lamar = $this->db->query("SELECT *, count(lamar.id_pencaker)as jml FROM lamar WHERE  lamar.id_lowongan='$row->id_lowongan' ")->row();
              ?>
                <!-- $q = $this->db->query("select * from lowongan WHERE lowongan.id_user='$id' AND lowongan.jenis_lowongan='Luar Negeri' and lowongan.tgl_tutup_loker<='$today' and lowongan.status='Tutup Lowongan'");
                            foreach ($q->result() as $row) {
                ...
                <tr align=center>
                  ...
                  <td width=25%>
                    <a class=" btn btn-primary" href="perusahaan/historipelamarln/<?php echo $row->id_lowongan ?>" role="button">
                      Pelamar<span class="badge badge-light">
                        <?php echo $lamar->jml ?></span></a>
                  </td>
                  <td>
                    <a href="perusahaan/detailhistorilokerln/<?php echo $row->id_lowongan ?>" class="btn btn-warning"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
              <?php $no++;
              } ?>
