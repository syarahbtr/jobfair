          ...
          <table class="table table-hover dttable">
            ...
            <tbody>
              ...
              $q = $this->db->query("select lamar.*,user.*,user.id_user as user_id,status_pelamar.nama_status_lamar,pencari_kerja.*,
                            ...
                            where lamar.id_lowongan='$id' and lowongan.jenis_lowongan='Dalam Negeri' ");
              foreach ($q->result() as $row) {
                
              ?>
              <?php 
              $k=$this->db->query("select lamar.*,lowongan.*,user.* from lamar
              left join user on user.id_user=lamar.id_user
              left join lowongan on lowongan.id_lowongan=lamar.id_lowongan
              where lamar.id_lowongan='$id'")->row();
              ?>
               <a href="perusahaan/eksportpdfpelamardn/<?php echo $k->id_lowongan ?>" class="btn btn-primary mb-3" target="_blank"><i class="fa fa-download"></i> Eksport PDF</a>
                <tr align=center>
                  <td><?php echo $no; ?></td>
                  <?php 
                  ...
                  <td>
                    <a href="perusahaan/detailhistoripelamardn/<?php echo $this->uri->segment(3).'/'.$row->user_id ?>" class="btn btn-warning"><?php echo $row->nama_status_lamar ?><i class="fa fa-eye"></i></a>
                  </td>
